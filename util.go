...
func printNodeDeepSubtree(ndb *nodeDB, node *Node, indent int) error {
	indentPrefix := ""
	for i := 0; i < indent; i++ {
		indentPrefix += "    "
	}
	...
	if node.rightNode != nil {
		printNodeDeepSubtree(ndb, node.rightNode, indent+1)
	}
	...
	if node.leftNode != nil {
		printNodeDeepSubtree(ndb, node.leftNode, indent+1)
	}
	return nil
}
